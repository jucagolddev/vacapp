  ...
  template: `
    <ion-header class="ion-no-border">
      <ion-toolbar>
        ...
        <ion-title class="ion-text-center">Recría & Rendimiento</ion-title>
      ...
      <ng-template>
        <div class="vac-filter-panel ion-padding">
          <div class="vac-filter-header mb-4">
            ...
            <span class="ml-2 font-bold color-primary">Filtrar Selección</span>
          </div>

          <!-- SECCIÓN: CATEGORÍA -->
          <div class="filter-section mb-6">
            <ion-label class="vac-filter-label">Categoría Zootécnica</ion-label>
            <ion-item lines="none" class="vac-input-select-mini">
              <ion-select [value]="filterCategoria()" (ionChange)="filterCategoria.set($event.detail.value)" interface="popover">
                ...
                <ion-select-option value="Ternero/a">Terneros</ion-select-option>
                <ion-select-option value="Añojo/a">Añojos</ion-select-option>
                <ion-select-option value="Novillo/a">Novillos</ion-select-option>
                <ion-select-option value="Vaca">Vacas Adultas</ion-select-option>
                <ion-select-option value="Toro">Toros</ion-select-option>
              </ion-select>
            </ion-item>
          </div>
          ...
          <!-- SECCIÓN: SEXO -->
          <div class="filter-section mb-6">
            <ion-label class="vac-filter-label">Género</ion-label>
            <ion-segment [value]="filterSexo()" (ionChange)="filterSexo.set($any($event).detail.value)" mode="ios" class="vac-segment-earth">
              <ion-segment-button value="Todos">
                <ion-label>Ambos</ion-label>
              </ion-segment-button>
              <ion-segment-button value="Macho">
                <ion-label>Machos</ion-label>
              </ion-segment-button>
              <ion-segment-button value="Hembra">
                <ion-label>Hembras</ion-label>
              </ion-segment-button>
            </ion-segment>
          </div>
...
export class RecriaComponent {
  ...
  filteredBovinos = computed(() => {
    ...
    const sexo = this.filterSexo();
    let list = this.ganadoService.bovinosAlta();
    ...
    if (sexo !== 'Todos') {
      list = list.filter(b => b.sexo === sexo);
    }

    if (cat !== 'Todos') {
      list = list.filter(b => this.ganadoService.calculateCategoria(b) === cat);
    }
